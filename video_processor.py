...
def get_binary(name: str) -> str:
    """Знаходить ffmpeg/ffprobe в системі або в робочій папці."""
...
def get_media_info(file_path: str):
    """Отримує точну тривалість, роздільну здатність та аудіодоріжки через ffprobe."""
    if not os.path.exists(file_path) or os.path.getsize(file_path) < 100:
        raise FileNotFoundError(f"Файл {file_path} відсутній або пошкоджений.")
    ...
    if result.returncode != 0 or not result.stdout:
        raise ValueError(f"Помилка читання медіафайлу: {result.stderr}")
    ...
    # Вирівнюємо розміри до парних чисел для H.264
...
def calculate_insertion_time(duration: float) -> float:
    """
    НОВЕ ПРАВИЛО:
    Банер завжди вставляється рівно посередині відео (duration / 2).
    """
    if duration <= 1.0:
        return duration / 2.0
    return duration / 2.0
...
def _sync_process_video(main_video_path: str, banner_video_path: str, output_path: str) -> str:
    ...
    if main_dur <= 0 or banner_dur <= 0:
        raise ValueError("Не вдалося визначити тривалість відео або банера.")
        
    # Завжди ділимо час відео навпіл (середина)
    t_insert = calculate_insertion_time(main_dur)
    if t_insert >= main_dur:
        t_insert = max(0.1, main_dur / 2.0)

    logger.info(f"Main video: {main_dur:.2f}s | Banner: {banner_dur:.2f}s | Insertion at: {t_insert:.2f}s (exactly half)")
    ...
    # 2. Пауза: заморожений кадр на моменті середини тривалістю banner_dur
    ...
    # 3. Банер: масштабуємо зі збереженням пропорцій і накладаємо по центру кадру
    ...
    # 6. Склеювання 3 частин (Частина 1 -> Пауза з банером -> Частина 2)
